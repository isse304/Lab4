    ...
    public class RelationalDatabase : IDatabase
    {
        ...
        public void AddEntry(Entry entry)
        {
            try
            {
                ...
                var sql = "INSERT INTO entries (id, clue, answer, difficulty, date) VALUES(@id, @clue, @answer, @difficulty, @date)";
        ...
        public bool DeleteEntry(Entry entry)
        {
            try
            {
                ...
                using var cmd = new NpgsqlCommand(sql, con);
                cmd.Parameters.AddWithValue("isbn", 1450);
                con.Close();
                return true;
            }
            catch (IOException ioe)
            ...
            foreach (Entry entry in entries) // iterate through entries until we find the Entry in question
            {
                if (entry.Id == replacementEntry.Id) // found it
                {
                    ...
                    try
                    {
                        ...
                        var sql = "UPDATE entries (id, clue, answer, difficulty, date) VALUES(@id, @clue, @answer, @difficulty, @date)";
                        using var cmd = new NpgsqlCommand(sql, con);
                        cmd.Parameters.AddWithValue("id", entry.Id);
                        cmd.Parameters.AddWithValue("clue", entry.Clue);
                        cmd.Parameters.AddWithValue("answer", entry.Answer);
        ...
        public String InitializeConnectionString()
        {
            ...
            var bitApiKey = "v2_3uhaz_3UKYzBNhHmkD6LrCjkSpeCG"; // from the "Password" field of the "Connect" menu

            var bitUser = "isse304";
            var bitDbName = "isse304/Lab3";
